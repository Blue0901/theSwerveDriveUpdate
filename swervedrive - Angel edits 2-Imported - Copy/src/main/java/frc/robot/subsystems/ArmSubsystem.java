...
public class ArmSubsystem extends SubsystemBase {
  ...
  private void updateMotionProfile() {
    ...
    m_profile = new TrapezoidProfile(Constants.Arm.kArmMotionConstraint, m_endState);// */
    m_timer.reset();
  }
  ...
  public void runAutomatic() {
    ...
    if (m_profile.isFinished(elapsedTime)) {
      ...
      m_targetState = m_profile.calculate(elapsedTime); //? with calculate
    }
  ...
  public void runManual(double _power) {
    ...
    m_motor.set(_power + (m_feedforward / 60.0)); // change from 12 to 60 to make slower!!
  ...
  public double  getPosition(){
    double x = m_encoder.getPosition();
    return x;
  }
